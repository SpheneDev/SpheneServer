...
namespace SpheneAuthService.Services;
...
public class SecretKeyAuthenticatorService
{
    ...
    public SecretKeyAuthenticatorService(SpheneMetrics metrics, IDbContextFactory<SpheneDbContext> dbContextFactory,
    ...
    private async Task<SecretKeyAuthReply> GetAuthReply(string ip, SpheneDbContext context, Auth? authReply)
    {
        ...
        // Check if User navigation property is null and handle gracefully
        string alias = string.Empty;
        if (authReply?.User != null)
        {
            alias = authReply.User.Alias ?? string.Empty;
        }
        else if (authReply != null)
        {
            _logger.LogWarning("Auth entry found but User navigation property is null. UserUID: {UserUID}", authReply.UserUID);
        }

        SecretKeyAuthReply reply = new(authReply != null, authReply?.UserUID,
            authReply?.PrimaryUserUID ?? authReply?.UserUID, alias,
