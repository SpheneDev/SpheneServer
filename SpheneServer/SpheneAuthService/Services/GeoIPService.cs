...
public class GeoIPService : IHostedService
{
    ...
    private readonly IConfigurationService<AuthServiceConfiguration> _spheneConfiguration;
    ...
    public GeoIPService(ILogger<GeoIPService> logger,
        IConfigurationService<AuthServiceConfiguration> spheneConfiguration)
    {
        _logger = logger;
        _spheneConfiguration = spheneConfiguration;
    }
    ...
    private async Task PeriodicReloadTask(CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            try
            {
                ...
                var useGeoIP = _spheneConfiguration.GetValueOrDefault(nameof(AuthServiceConfiguration.UseGeoIP), false);
                var cityFile = _spheneConfiguration.GetValueOrDefault(nameof(AuthServiceConfiguration.GeoIPDbCityFile), string.Empty);
