...
public class OAuthController : AuthControllerBase
{
    ...
    IHttpContextAccessor accessor, IDbContextFactory<SpheneDbContext> SpheneDbContext,
    SecretKeyAuthenticatorService secretKeyAuthenticatorService,
    IConfigurationService<AuthServiceConfiguration> configuration,
    IDatabase redisDb, GeoIPService geoIPProvider)
        : base(logger, accessor, SpheneDbContext, secretKeyAuthenticatorService,
    ...
    public async Task<IActionResult> DiscordOAuthCallback([FromQuery] string code)
    {
        ...
        var spheneUser = await dbContext.LodeStoneAuth.Include(u => u.User).SingleOrDefaultAsync(u => u.DiscordId == discordUserId);
        if (spheneUser == default)
        {
            Logger.LogDebug("Failed to get Sphene user for {session}, DiscordId: {id}", reqId, discordUserId);

            return BadRequest("Could not find a Sphene user associated to this Discord account.");
        }
        ...
        {
            jwt = CreateJwt([
                new Claim(SpheneClaimTypes.Uid, spheneUser.User!.UID),
                new Claim(SpheneClaimTypes.Expires, DateTime.UtcNow.AddDays(14).Ticks.ToString(CultureInfo.InvariantCulture)),
                new Claim(SpheneClaimTypes.DiscordId, discordUserId.ToString()),
                new Claim(SpheneClaimTypes.DiscordUser, discordUserName),
                new Claim(SpheneClaimTypes.OAuthLoginToken, true.ToString())
            ]);
        }
        catch (Exception ex)
        {
    ...
    [HttpPost(SpheneAuth.OAuth_GetUIDsBasedOnSecretKeys)]
    ...
    [Authorize(Policy = "OAuthToken")]
    [HttpPost(SpheneAuth.OAuth_RenewOAuthToken)]
    public IActionResult RenewOAuthToken()
    {
        var claims = HttpContext.User.Claims.Where(c => c.Type != SpheneClaimTypes.Expires).ToList();
            claims.Add(new Claim(SpheneClaimTypes.Expires, DateTime.UtcNow.AddDays(14).Ticks.ToString(CultureInfo.InvariantCulture)));
        return Content(CreateJwt(claims).RawData);
    }

    [AllowAnonymous]
    [HttpGet(SpheneAuth.OAuth_GetDiscordOAuthToken)]
    ...
    [AllowAnonymous]
    [HttpGet(SpheneAuth.OAuth_GetDiscordOAuthEndpoint)]
    ...
    [Authorize(Policy = "OAuthToken")]
    [HttpGet(SpheneAuth.OAuth_GetUIDs)]
    public async Task<Dictionary<string, string>> GetAvailableUIDs()
    {
        string primaryUid = HttpContext.User.Claims.Single(c => string.Equals(c.Type, SpheneClaimTypes.Uid, StringComparison.Ordinal))!.Value;
        using var dbContext = await SpheneDbContextFactory.CreateDbContextAsync();

        var spheneUser = await dbContext.Auth.AsNoTracking().Include(u => u.User).FirstOrDefaultAsync(f => f.UserUID == primaryUid).ConfigureAwait(false);
        if (spheneUser == null || spheneUser.User == null) return [];
        var uid = spheneUser.User.UID;
    ...
    [Authorize(Policy = "OAuthToken")]
    [HttpPost(SpheneAuth.OAuth_CreateOAuth)]
    ...
    private async Task<IActionResult> AuthenticateOAuthInternal(SpheneDbContext dbContext, string requestedUid, string charaIdent)
    {
        try
        {
            string primaryUid = HttpContext.User.Claims.Single(c => string.Equals(c.Type, SpheneClaimTypes.Uid, StringComparison.Ordinal))!.Value;
