using SpheneAuthService.Authentication;
...
public abstract class AuthControllerBase : Controller
{
    ...
    protected readonly IDbContextFactory<SpheneDbContext> SpheneDbContextFactory;
    ...
    IDatabase redisDb, GeoIPService geoIPProvider)
    {
        ...
        SpheneDbContextFactory = spheneDbContextFactory;
        SecretKeyAuthenticatorService = secretKeyAuthenticatorService;
        Configuration = configuration;
    }

    protected async Task<IActionResult> GenericAuthResponse(SpheneDbContext dbContext, string charaIdent, SecretKeyAuthReply authResult)
    {
        ...
        if (!authResult.Success && !authResult.TempBan)
        {
            ...
            return Unauthorized("The provided secret key is invalid. Verify your Sphene accounts existence and/or recover the secret key.");
        }
        if (!authResult.Success && authResult.TempBan)
        {
        ...
        if (authResult.Permaban || authResult.MarkedForBan)
        {
            ...
            return Unauthorized("Your Sphene account is banned from using the service.");
        }
        ...
        if (!string.IsNullOrEmpty(existingIdent))
        {
            ...
            return Unauthorized("Already logged in to this Sphene account. Reconnect in 60 seconds. If you keep seeing this issue, restart your game.");
        }
    ...
    {
        var authSigningKey = new SymmetricSecurityKey(Encoding.ASCII.GetBytes(Configuration.GetValue<string>(nameof(SpheneConfigurationBase.Jwt))));

        var token = new SecurityTokenDescriptor()
        {
            Subject = new ClaimsIdentity(authClaims),
            SigningCredentials = new SigningCredentials(authSigningKey, SecurityAlgorithms.HmacSha256Signature),
            Expires = new(long.Parse(authClaims.First(f => string.Equals(f.Type, SpheneClaimTypes.Expires, StringComparison.Ordinal)).Value!, CultureInfo.InvariantCulture), DateTimeKind.Utc),
        };
    ...
    protected async Task<IActionResult> CreateJwtFromId(string uid, string charaIdent, string alias)
    {
        ...
        {
            new Claim(SpheneClaimTypes.Uid, uid),
            new Claim(SpheneClaimTypes.CharaIdent, charaIdent),
            new Claim(SpheneClaimTypes.Alias, alias),
            new Claim(SpheneClaimTypes.Expires, DateTime.UtcNow.AddHours(6).Ticks.ToString(CultureInfo.InvariantCulture)),
            new Claim(SpheneClaimTypes.Continent, await _geoIPProvider.GetCountryFromIP(HttpAccessor))
        });
    ...
    protected async Task<bool> IsIdentBanned(SpheneDbContext dbContext, string charaIdent)
    {
