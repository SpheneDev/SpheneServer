...
public class ExistingUserRequirementHandler : AuthorizationHandler<ExistingUserRequirement>
{
    ...
    public ExistingUserRequirementHandler(IDbContextFactory<SpheneDbContext> dbContext, ILogger<ExistingUserRequirementHandler> logger)
    {
    ...
    protected override async Task HandleRequirementAsync(AuthorizationHandlerContext context, ExistingUserRequirement requirement)
    {
        try
        {
            var uid = context.User.Claims.SingleOrDefault(g => string.Equals(g.Type, SpheneClaimTypes.Uid, StringComparison.Ordinal))?.Value;
            if (uid == null)
            {
                context.Fail();
            ...
            var discordIdString = context.User.Claims.SingleOrDefault(g => string.Equals(g.Type, SpheneClaimTypes.DiscordId, StringComparison.Ordinal))?.Value;
            ...
            {
                _logger.LogWarning("Failed to find Sphene User {User} in DB", uid);
                context.Fail();
                return;
            }
