...
public class SpheneConfigurationServiceClient<T> : IHostedService, IConfigurationService<T> where T : class, ISpheneConfiguration
{
    ...
    private readonly ILogger<SpheneConfigurationServiceClient<T>> _logger;
    ...
    private Uri GetRoute(string key, string value)
    {
        if (_config.CurrentValue.GetType() == typeof(ServerConfiguration))
            return new Uri((_config.CurrentValue as ServerConfiguration).MainServerAddress, $"configuration/SpheneServerConfiguration/{nameof(SpheneServerConfigurationController.GetConfigurationEntry)}?key={key}&defaultValue={value}");
        if (_config.CurrentValue.GetType() == typeof(SpheneConfigurationBase))
            return new Uri((_config.CurrentValue as SpheneConfigurationBase).MainServerAddress, $"configuration/SpheneBaseConfiguration/{nameof(SpheneBaseConfigurationController.GetConfigurationEntry)}?key={key}&defaultValue={value}");
        if (_config.CurrentValue.GetType() == typeof(ServicesConfiguration))
            return new Uri((_config.CurrentValue as ServicesConfiguration).MainServerAddress, $"configuration/SpheneServicesConfiguration/{nameof(SpheneServicesConfigurationController.GetConfigurationEntry)}?key={key}&defaultValue={value}");
        if (_config.CurrentValue.GetType() == typeof(StaticFilesServerConfiguration))
            return new Uri((_config.CurrentValue as StaticFilesServerConfiguration).MainFileServerAddress, $"configuration/SpheneStaticFilesServerConfiguration/{nameof(SpheneStaticFilesServerConfigurationController.GetConfigurationEntry)}?key={key}&defaultValue={value}");

        throw new NotSupportedException("Config is not supported to be gotten remotely");
    }

    public SpheneConfigurationServiceClient(ILogger<SpheneConfigurationServiceClient<T>> logger, IOptionsMonitor<T> config, ServerTokenGenerator serverTokenGenerator)
    {
        _config = config;
        _logger = logger;
    ...
    {
        _logger.LogInformation("Starting SpheneConfigurationServiceClient");
