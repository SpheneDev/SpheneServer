...
public class OnlineSyncedPairCacheService
{
    ...
    public OnlineSyncedPairCacheService(ILogger<OnlineSyncedPairCacheService> logger, ILoggerFactory loggerFactory, SpheneMetrics SpheneMetrics)
    {
        _logger = logger;
    ...
    public async Task InitPlayer(string user)
    {
        ...
        try
        {
            ...
            _lastSeenCache[user] = new(_loggerFactory.CreateLogger<PairCache>(), user, _SpheneMetrics);
        }
        finally
        {
    ...
    private sealed class PairCache : IDisposable
    {
        ...
        public PairCache(ILogger<PairCache> logger, string owner, SpheneMetrics metrics)
        {
