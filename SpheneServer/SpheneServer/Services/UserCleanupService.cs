...
{
    private readonly SpheneMetrics metrics;
    private readonly ILogger<UserCleanupService> _logger;
    private readonly IDbContextFactory<SpheneDbContext> _SpheneDbContextFactory;
    private readonly IConfigurationService<ServerConfiguration> _configuration;
    private CancellationTokenSource _cleanupCts;

    public UserCleanupService(SpheneMetrics metrics, ILogger<UserCleanupService> logger, IDbContextFactory<SpheneDbContext> spheneDbContextFactory, IConfigurationService<ServerConfiguration> configuration)
    {
        this.metrics = metrics;
        _logger = logger;
        _SpheneDbContextFactory = spheneDbContextFactory;
    ...
    private async Task CleanUp(CancellationToken ct)
    {
        ...
        {
            using (var dbContext = await _SpheneDbContextFactory.CreateDbContextAsync(ct).ConfigureAwait(false))
            {
    ...
    private async Task PurgeTempInvites(SpheneDbContext dbContext)
    {
        try
        {
    ...
    private async Task PurgeUnusedAccounts(SpheneDbContext dbContext)
    {
        try
        {
    ...
    private void CleanUpOutdatedLodestoneAuths(SpheneDbContext dbContext)
    {
        try
        {
    ...
    public async Task PurgeUser(User user, SpheneDbContext dbContext)
    {
