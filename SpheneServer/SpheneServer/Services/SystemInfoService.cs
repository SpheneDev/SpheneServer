...
public sealed class SystemInfoService : BackgroundService
{
    ...
    private readonly IHubContext<SpheneHub, ISpheneHub> _hubContext;
    private readonly IRedisDatabase _redis;
    public SystemInfoDto SystemInfoDto { get; private set; } = new();

    public SystemInfoService(SpheneMetrics SpheneMetrics, IConfigurationService<ServerConfiguration> configurationService, IDbContextFactory<SpheneDbContext> dbContextFactory,
        ILogger<SystemInfoService> logger, IHubContext<SpheneHub, ISpheneHub> hubContext, IRedisDatabase redisDb)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            try
            {
                ...
                _SpheneMetrics.SetGaugeTo(MetricsAPI.GaugeAvailableWorkerThreads, workerThreads);
                _SpheneMetrics.SetGaugeTo(MetricsAPI.GaugeAvailableIOWorkerThreads, ioThreads);
                ...
                if (_config.IsMain)
                {
                    ...
                    _SpheneMetrics.SetGaugeTo(MetricsAPI.GaugeAuthorizedConnections, onlineUsers);
                    _SpheneMetrics.SetGaugeTo(MetricsAPI.GaugePairs, db.ClientPairs.AsNoTracking().Count());
                    _SpheneMetrics.SetGaugeTo(MetricsAPI.GaugePairsPaused, db.Permissions.AsNoTracking().Where(p => p.IsPaused).Count());
                    _SpheneMetrics.SetGaugeTo(MetricsAPI.GaugeGroups, db.Groups.AsNoTracking().Count());
                    _SpheneMetrics.SetGaugeTo(MetricsAPI.GaugeGroupPairs, db.GroupPairs.AsNoTracking().Count());
                    _SpheneMetrics.SetGaugeTo(MetricsAPI.GaugeUsersRegistered, db.Users.AsNoTracking().Count());
                }
