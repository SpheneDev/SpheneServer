...
{
    public string UserCharaIdent => Context.User?.Claims?.SingleOrDefault(c => string.Equals(c.Type, SpheneClaimTypes.CharaIdent, StringComparison.Ordinal))?.Value ?? throw new Exception("No Chara Ident in Claims");

    public string UserUID => Context.User?.Claims?.SingleOrDefault(c => string.Equals(c.Type, SpheneClaimTypes.Uid, StringComparison.Ordinal))?.Value ?? throw new Exception("No UID in Claims");

    public string Continent => Context.User?.Claims?.SingleOrDefault(c => string.Equals(c.Type, SpheneClaimTypes.Continent, StringComparison.Ordinal))?.Value ?? "UNK";
    ...
    private async Task DeleteUser(User user)
    {
        ...
        _SpheneMetrics.IncCounter(MetricsAPI.CounterUsersRegisteredDeleted, 1);
    ...
    private async Task<Dictionary<string, string>> GetOnlineUsers(List<string> uids)
    {
        ...
        var online = new Dictionary<string, string>(StringComparer.Ordinal);
        foreach (var uid in uids)
        {
            if (!result.TryGetValue("UID:" + uid, out var ident) || string.IsNullOrEmpty(ident))
            {
                continue;
            }
            if (!_userConnections.ContainsKey(uid))
            {
                continue;
            }
            online[uid] = ident;
    ...
    private async Task UserLeaveGroup(GroupDto dto, string userUid)
    {
        ...
        if (ownerHasLeft)
        {
            ...
            {
                _logger.LogCallInfo(SpheneHubLogger.Args(dto, "Deleted"));
            ...
            else
            {
                ...
                {
                    _logger.LogCallInfo(SpheneHubLogger.Args(dto, "Migrated", groupHasMigrated.Item2));
