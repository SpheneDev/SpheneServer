...
public partial class SpheneHub
{
    ...
    {
        _SpheneMetrics.IncGauge(MetricsAPI.GaugeGposeLobbyUsers);
        if (priorUsers.Count == 0)
            _SpheneMetrics.IncGauge(MetricsAPI.GaugeGposeLobbies);
    ...
    private async Task RemoveUserFromLobby(string lobbyId, List<string> priorUsers)
    {
        ...
        _SpheneMetrics.DecGauge(MetricsAPI.GaugeGposeLobbyUsers);

        if (priorUsers.Count == 1)
        {
            await _redis.RemoveAsync($"GposeLobby:{lobbyId}").ConfigureAwait(false);
            _SpheneMetrics.DecGauge(MetricsAPI.GaugeGposeLobbies);
        }
        else
        {
