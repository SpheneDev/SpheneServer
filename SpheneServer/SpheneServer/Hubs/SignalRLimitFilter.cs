...
public class SignalRLimitFilter : IHubFilter
{
        ...
        HubInvocationContext invocationContext, Func<HubInvocationContext, ValueTask<object>> next)
    {
        ...
        foreach (var rule in await _processor.GetMatchingRulesAsync(client).ConfigureAwait(false))
        {
            ...
            {
                var authUserId = invocationContext.Context.User.Claims?.SingleOrDefault(c => string.Equals(c.Type, SpheneClaimTypes.Uid, StringComparison.Ordinal))?.Value ?? "Unknown";
