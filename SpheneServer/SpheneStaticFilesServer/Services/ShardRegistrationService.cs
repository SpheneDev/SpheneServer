...
public class ShardRegistrationService : IHostedService
{
    ...
    private async Task ShardHeartbeat(CancellationToken ct)
    {
        ...
        using var heartBeat = await _httpClient.PostAsync(new Uri(mainServer, SpheneFiles.Main + "/shardHeartbeat"), null, ct).ConfigureAwait(false);
    ...
    private async Task TryRegisterShard(CancellationToken ct)
    {
        ...
        using var register = await _httpClient.PostAsJsonAsync(new Uri(mainServer, SpheneFiles.Main + "/shardRegister"), config, ct).ConfigureAwait(false);
    ...
    private async Task UnregisterShard()
    {
        ...
        using var heartBeat = await _httpClient.PostAsync(new Uri(mainServer, SpheneFiles.Main + "/shardUnregister"), null).ConfigureAwait(false);
    }
}
