...
public class MainFileCleanupService : IHostedService
{
    ...
    public MainFileCleanupService(SpheneMetrics metrics, ILogger<MainFileCleanupService> logger,
    ...
    private List<FileInfo> CleanUpFilesBeyondSizeLimit(List<FileInfo> files, double sizeLimit, bool deleteFromDb, SpheneDbContext dbContext, CancellationToken ct)
    {
        if (sizeLimit <= 0)
        {
            return [];
    ...
    private void CleanUpStuckUploads(SpheneDbContext dbContext)
    {
