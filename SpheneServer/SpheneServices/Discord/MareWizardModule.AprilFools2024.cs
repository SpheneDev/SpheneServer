...
public partial class SpheneWizardModule : InteractionModuleBase
{
    ...
    public async Task ComponentFools()
    {
        ...
        EmbedBuilder eb = new();
        eb.WithTitle("WorryCoin� and SpheneToken� Balance");
    ...
    public async Task ComponentFoolsStart()
    {
        ...
        if (userIsInPermanentVanityRole)
        {
            ...
            eb.WithDescription("Thank you for participating in Sphenes 2024 April Fools event."
        ...
        else if (remainingRoles > 0)
        {
            ...
            eb.WithDescription("Thank you for participating in Sphenes 2024 April Fools event."
                ...
                + Environment.NewLine + Environment.NewLine
                + "The giveaway is active until <t:" + (new DateTime(2024, 04, 01, 23, 59, 59, DateTimeKind.Utc).Subtract(DateTime.UnixEpoch).TotalSeconds) + ":f>.");
            cb.WithButton("Win", "wizard-fools-win:1", ButtonStyle.Primary, new Emoji("1??"));
            cb.WithButton("Win", "wizard-fools-win:2", ButtonStyle.Primary, new Emoji("2??"));
            cb.WithButton("Win", "wizard-fools-win:3", ButtonStyle.Primary, new Emoji("3??"));
            cb.WithButton("Win", "wizard-fools-win:4", ButtonStyle.Primary, new Emoji("4??"));
            cb.WithButton("Win", "wizard-fools-win:5", ButtonStyle.Primary, new Emoji("5??"));
        }
        else
        {
            eb.WithColor(Color.Orange);
    ...
    public async Task ComponentFoolsWin(int number)
    {
        ...
        if (hasWon)
        {
            ...
            var user = await Context.Guild.GetUserAsync(Context.User.Id).ConfigureAwait(false);
            await user.AddRoleAsync(_spheneServicesConfiguration.GetValue<ulong?>(nameof(ServicesConfiguration.DiscordRoleAprilFools2024)).Value).ConfigureAwait(false);
        }
        else
        {
