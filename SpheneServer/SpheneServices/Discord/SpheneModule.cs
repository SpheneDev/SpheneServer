...
public class SpheneModule : InteractionModuleBase
{
    private readonly ILogger<SpheneModule> _logger;
    private readonly IServiceProvider _services;
    private readonly IConfigurationService<ServicesConfiguration> _spheneServicesConfiguration;
    private readonly IConnectionMultiplexer _connectionMultiplexer;

    public SpheneModule(ILogger<SpheneModule> logger, IServiceProvider services,
        ...
        [Summary("uid", "User ID to the person to send the message to")] string? uid = null)
    {
        ...
        using var db = scope.ServiceProvider.GetService<SpheneDbContext>();
        ...
        try
        {
            ...
            await c.PostAsJsonAsync(new Uri(_spheneServicesConfiguration.GetValue<Uri>
            ...
            var discordChannelForMessages = _spheneServicesConfiguration.GetValueOrDefault<ulong?>(nameof(ServicesConfiguration.DiscordChannelForMessages), null);
    ...
    private async Task<EmbedBuilder> HandleUserInfo(EmbedBuilder eb, ulong id, string? secondaryUserUid = null, ulong? optionalUser = null, string? uid = null)
    {
        ...
        using var scope = _services.CreateScope();
        await using var db = scope.ServiceProvider.GetRequiredService<SpheneDbContext>();
        ...
        if (primaryUser == null)
        {
            ...
            eb.WithDescription("No Sphene account was found associated to your Discord user");
            return eb;
        }
        ...
        else if ((optionalUser != null || uid != null) && isAdminCall)
        {
            ...
            {
                eb.WithTitle("No account");
                eb.WithDescription("The Discord user has no valid Sphene account");
                return eb;
            }
