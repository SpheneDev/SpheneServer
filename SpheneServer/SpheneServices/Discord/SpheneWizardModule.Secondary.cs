...
public partial class SpheneWizardModule
{
    ...
    public async Task ComponentSecondary()
    {
        ...
        var primaryUID = (await spheneDb.LodeStoneAuth.Include(u => u.User).SingleAsync(u => u.DiscordId == Context.User.Id).ConfigureAwait(false)).User.UID;
        var secondaryUids = await spheneDb.Auth.CountAsync(p => p.PrimaryUserUID == primaryUID).ConfigureAwait(false);
        EmbedBuilder eb = new();
    ...
    public async Task ComponentSecondaryCreate(string primaryUid)
    {
        ...
        ComponentBuilder cb = new();
        AddHome(cb);
        await HandleAddSecondary(spheneDb, eb, primaryUid).ConfigureAwait(false);
        await ModifyInteraction(eb, cb).ConfigureAwait(false);
    }

    public async Task HandleAddSecondary(SpheneDbContext db, EmbedBuilder embed, string primaryUID)
    {
        User newUser = new()
        {
            IsAdmin = false,
